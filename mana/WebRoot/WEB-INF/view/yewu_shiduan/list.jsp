	...
	function getdata(url) {
		...
		datagrid=$("#dg").datagrid({  
	        ...
	        onDblClickRow: function (index, row) {
	        	...
	        	getdata2("yewu/daili_xiangqingdata?bianhao="+row.bianhao);
	        	$('#dlg').dialog('open');
	        },
	   });
	}
...
function search1() {
	$.ajax({
	    type : "POST",
	    url : "<%=request.getContextPath()%>/yewu/daili_search",
	    data : {
...
function deletedata(bianhao) {
	if(confirm('确定删除吗 ？')) {
		$.ajax({
		    type : "POST",
		    url : "<%=request.getContextPath()%>/yewu/daili_deletedata",
		    data : {
		      'bianhao' : bianhao,
		     },
		    dataType: "json",

...
$(document).ready(function() { 
	getdata("kefu/htchuanmei_listdata");
	function getdata(url) {
		...
		datagrid=$("#dg").datagrid({  
	        ...
	        title:"数据列表 -> 只显示最近的20条需要审核的数据，更多数据请根据条件查询！双击行进行合同审核。",  
	        columns:[[      //每个列具体内容  
	        ...
	        onDblClickRow: function (index, row) {
	        	//alert(index +":"+row.id);
	        	self.location='kefu/htchuanmei_shenhe?id=' + row.id;
	        },
	   });
	}
});
function search1() {
	$.ajax({
	    type : "POST",
	    url : "<%=request.getContextPath()%>/kefu/htchuanmei_search",
	    data : {

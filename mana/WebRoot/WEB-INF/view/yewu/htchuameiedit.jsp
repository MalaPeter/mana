...
<body>
	<form id="ff" method="post" data-options="novalidate:true">
		<div id="p" class="easyui-panel" title="合同录入-广电传媒合同录入单" style="width:800px;height:580px;padding:1px;text-align: center;" fit="true">
		<h1>吉林广电传媒有限责任公司</h1>
		<h2>广告发布合同 - 品牌(PP)</h2>
		<p align="right">合同编号：<font id="bianhao"></font></p>
			...
			<a href="javascript:void(0)" class="easyui-linkbutton" onclick="submitForm()">修改合同</a>
			<a href="javascript:void(0)" class="easyui-linkbutton" onclick="initpage()">调取数据</a>
		</div>
	</form>
	<!-- 输出对话框 -->
	<div id="dlg" class="easyui-dialog" title="行业查询" style="width:400px;height:600px;padding:10px"
			data-options="
				buttons: [{
					text:'继续',
					iconCls:'icon-ok',
...
function sub() {
	...
	$.ajax( {   
	    type : "POST",
	    url : "<%=request.getContextPath()%>/yewu/ht_chuanmei_yingxing_edit?id=${model.id}", 
	    data : {
	      'bianhao' : bianhao,
	      'kehuname' : kehuname,
	      'hangye' : hangye,
...
//页面内容初始化
function initpage() {
	//要触发选中事件，所以不能使用setValue
	$('#kehuname').combobox('select','${model.kehuname}');//设置客户名称
	$('#hangye').combobox('select','${model.hangye}');//设置客户行业
	$('#sdate').datebox('setValue', '${model.sdate}');
	$('#edate').datebox('setValue', '${model.edate}');
	$('#J_input').val('${model.nodate}');
	$('#meiti').combobox('select','${model.meiti}');
	$('#guige').combobox('select','${model.guige}');
	
	var shiduanarray = new Array();
	shiduanarray = '${model.shiduan}'.split(','); 
	var zsshiduanarray = new Array();
	zsshiduanarray = '${model.zsshiduan}'.split(',');
	$("[name = shiduan]:checkbox").each(function () {//循环一次 当前的所有 时段
		if($.inArray($(this).val(),shiduanarray) >-1) {
			$(this).attr("checked","checked");
			dayandprice();
		}
		if($.inArray($(this).val(),zsshiduanarray) >-1) {
			$(this).attr("checked","checked");
			dayandprice();
		}
	});
	$(":radio[name='zhekou'][value='${model.zhekou}']").attr("checked",'checked');
	$(":radio[name='fkfangshi'][value='${model.fkfangshi}']").attr("checked",'checked');
	$('#fkyddate').datebox('setValue', '${model.fkyddate}');
	$('#beizhu').val('${model.beizhu}');
}
//页面内容初始化

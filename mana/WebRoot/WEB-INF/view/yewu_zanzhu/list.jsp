...
$(document).ready(function() { 
	$('#dlg').dialog('close');
	getdata("yewu_zanzhu/zanzhu_listdata");
	function getdata(url) {
		...
		datagrid=$("#dg").datagrid({  
	        ...
	        columns:[[      //每个列具体内容  
		        ...
		        {field:'sdate',title:'登记时间',width:100,align:'center'},
	        ...
	        onDblClickRow: function (index, row) {
	        	...
	        	//getdata2("yewu_pinpai/pinpai_xiangqingdata?bianhao="+row.bianhao);
	        	$('#dlg').dialog('open');
	        },
	   });
	}
...
function search1() {
	$.ajax({
	    type : "POST",
	    url : "<%=request.getContextPath()%>/yewu_zanzhu/zanzhu_search",
	    data : {
...
function deletedata(bianhao) {
	if(confirm('确定删除吗 ？')) {
		$.ajax({
		    type : "POST",
		    url : "<%=request.getContextPath()%>/yewu_zanzhu/zanzhu_del",
		    data : {
		      'bianhao' : bianhao,
		     },
		    dataType: "text",

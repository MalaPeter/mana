...
<html>
  <head>
	...
	var kehuname = "";//客户名称
  	...
  	$(document).ready(function() {
		...
		$.ajax( {   
		    type : "POST",
		    url : "<%=request.getContextPath()%>/publicf/getKehuname_daili", 
		    data : {
		...
		$.ajax({
		    type : "POST",
		    url : "<%=request.getContextPath()%>/publicf/getYewuyuan_daili",
		    dataType: "json",
		    success : function(data) {
		...
		$.ajax({
		    type : "POST",
		    url : "<%=request.getContextPath()%>/publicf/getDaili_daili",
		    dataType: "json",
		    success : function(data) {
				$("#daili").combobox("loadData", data);
		    },   
		    error :function(){
		        alert("网络连接出错！请联系网络管理员或服务器管理员！");
		    }
		...
		function gethetongsize() {
			$.ajax( {   
			    type : "POST",
			    url : "<%=request.getContextPath()%>/yewu_daili/daili_getsize", 
			    dataType: "text",   
			    success : function(data) {
	...
	function savetr(trflag) {
		...
		if(!tt == "") {
			...
			$.ajax( {   
			    type : "POST",
			    url : "<%=request.getContextPath()%>/yewu_daili/saveshiduan",
			    data : {
	...
	function deletetr(trflag) {
			$.ajax( {   
			    type : "POST",
			    url : "<%=request.getContextPath()%>/yewu_daili/delshiduan",
			    data : {
			      'nianfen' : $("#nianfen"+trflag).val(),
			      'yuefen' : $("#yuefen"+trflag).val(),
			      'shiduan' : $("#shiduan"+trflag).val(),
  ...
  <body>
   	<div id="p" class="easyui-panel" title="合同录入-广电传媒合同录入单" style="width:800px;height:800px;padding:1px;text-align: center;" fit="true">
		<h1>广告发布合同 - 代理(DL)</h1>
		<h2><select class="easyui-combobox" id="daili" name="daili" style="width: 300px"></select></h2>
